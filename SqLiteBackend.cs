...
namespace mdb2sq3
{
    ...
    class SQLiteBackend: IDBBackEnd
    {
        ...
        private void InsertViewInDB(string name, string query)
        {
            using (SQLiteConnection connector = new SQLiteConnection(connectionString))
            {
                StringBuilder stmtBuilder = new StringBuilder();
                stmtBuilder.Append("CREATE VIEW ");
                stmtBuilder.Append(name);
                stmtBuilder.Append(" AS ");
                stmtBuilder.Append(query);

                connector.Open();
                using (SQLiteCommand Cmd = new SQLiteCommand(stmtBuilder.ToString(), connector))
                {
                    Cmd.ExecuteNonQuery();
                }
                connector.Close();
